...
public class Viewer implements ViewerService, RequireReadService{
	...
	private ImageView nuageImageView;
	...
	public void init(){
		...
		nuageImage = new Image("file:src/images/nuage.png");
		nuageImageView = new ImageView(nuageImage);
		img = new Image("file:src/images/back.png");
	...
	public Parent getPanel(){
		...
		Text vie = new Text(0,-40+HardCodedParameters.defaultHeight, "Vies :"+data.getHeroesPointVie());
	    vie.setFont(new Font(20));

		/*
		 * Text lvl = new Text(-0.1*shrink*defaultMainHeight+.5*shrink*defaultMainWidth,
				-0.0125*shrink*defaultMainWidth+shrink*defaultMainHeight,
				"Level: 1");
		lvl.setFont(new Font(.05*shrink*defaultMainHeight));
		*/
		...
		nuageImageView.setFitHeight(90*shrink);
		nuageImageView.setPreserveRatio(true);
		nuageImageView.setTranslateX(-0.1*shrink*defaultMainHeight+.5*shrink*defaultMainWidth);
		nuageImageView.setTranslateY(shrink/0.1);

		Group panel = new Group();
		panel.getChildren().addAll(map,greets,vie,score,nuageImageView,heroesAvatar);

		ArrayList<PhantomService> phantoms = data.getPhantoms();
		PhantomService p;
		ArrayList<EnnemieService> ennemie = data.getEnnemie();
		EnnemieService e;
		...
		for (int i=0; i<ennemie.size();i++){
			e=ennemie.get(i);
			double radius=.5*Math.min(shrink*data.getEnnemieWidth(),shrink*data.getEnnemieHeight());
			Circle EnnemieAvatar = new Circle(radius,Color.rgb(255,0,0));
			EnnemieAvatar.setEffect(new Lighting());
			EnnemieAvatar.setTranslateX(shrink*e.getPosition().x+shrink*xModifier-radius);
			EnnemieAvatar.setTranslateY(shrink*e.getPosition().y+shrink*yModifier-radius);
